...
describe("App", () => {
  ...
  it("keeps preparation visible when the start write fails", async () => {
    ...
    expect(screen.getByRole("heading", { name: "Plan my workshop" })).toBeInTheDocument();
  ...
  it("closes a destructive dialog when a failed checkpoint disables live controls", async () => {
    ...
    expect(screen.getByRole("button", { name: "Resume" })).toBeDisabled();
    expect(screen.queryByRole("heading", { name: "Plan my workshop" })).not.toBeInTheDocument();
